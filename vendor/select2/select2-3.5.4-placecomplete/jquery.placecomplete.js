...
var GooglePlacesAPI = {
    ...
    getDetails: function(abbreviatedPlaceResult) {
        console.log(abbreviatedPlaceResult);
        return this.deferred.then($.proxy(function() {
            var deferred = new $.Deferred();
    ...
    var select2options = $.extend({}, {
        query: function(query) {
            GooglePlacesAPI.getPredictions(query.term, requestParams)
                .done(function(aprs) {
                    /*
                    GooglePlacesAPI.getDetails(apr)
                        .done(function(placeResult) {
                            aprs[i]["text"] = aprs[i]["description"];
                            aprs[i]['id'] = placeResult.geometry.location.lat()+','+placeResult.geometry.location.lng();
                            
                            aprs[i]["text"] = aprs[i]["description"];
                            
                            callback(i, (aprs.length -1), aprs);
                        });
                    */
        ...
        initSelection: function(element, callback) {
            ...
            if(initID) {
                ...
                GooglePlacesAPI.getDetails({place_id : initID, description : 'NULL'})
                .done(function(placeResult) {
                    
                    var addr = placeResult.address_components;
                    addr = jQuery.map( addr, function( a ) {
                        return a.long_name;
                    });
                    ...
                    callback({id: placeResult.place_id, text: addr });
                });
